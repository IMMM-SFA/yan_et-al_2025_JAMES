...
sim = readtable('../gscd_conus_regression/q10.csv');
...
default_sim = load('../default_sim/q90');
...
ensemble_q90_400 = load('../../conus_ensemble_sim/q90');
ensemble_q90_350 = nan(350, 50629);

count = 1;
for i = 1:400
    isValueInArray = ismember(para_id_400(i), para_id_350);
    if isValueInArray
        ensemble_q90_350(count,:) = ensemble_q90_400(i,:);
        count = count + 1;
    end
end
clear ensemble_q90_400;

para_q90 = load('conus_regional_id_350_q90.txt');




% find GSCD regression
temp = readtable('../gscd_conus_regression/q90.csv');
...
for i = 1:50629
    low = var(i,3);
    up  = var(i,4);
    indices = find(ensemble_q90_350(:,i) > low & ensemble_q90_350(:,i) < up);  % 1- 350

    % 1) full constrain
    if length(indices)>=10
        for j = 1:length(indices)
            out_sim_regression(i,j) = ensemble_q90_350(indices(j),i);
    ...
    if length(indices)==0
        ...
        for k = 1:7
            if cluster_id(i)==k
                ...
                for j = 1:10
                    out_sim_regression(i,j) = ensemble_q90_350(par_id(j),i);
            ...
            if cluster_id(i)==k
                ...
                for j = 1:20
                    if ismember(par_id(j), indices)
                        ...
                        out_sim_regression(i,count1) = ensemble_q90_350(par_id(j),i);
...
%         out_qmean_metric(i,9) = mean(temp2) - out_obs(i,7);
        out_q90_metric(i,9) = mean(temp2) - out_default(i,6);
        out_q90_metric(i,10) = std(temp2)/mean(temp2);
...
colorbar
caxis([0 12]); 
title('(a1) Q90 (mm/d): Default Parameter', 'FontSize', title_size);
set(gca,'FontSize',tick_size);
...
title('(b1) Q90 (mm/d): Default Parameter', 'FontSize', title_size);
xticks = [-120,-110,-100,-90,-80, -70];
xticklabels_deg = cell(size(xticks));
for i = 1:numel(xticks)
    xticklabels_deg{i} = [num2str(-xticks(i)) '°W'];
...
colorbar
caxis([-4 4]); 
title('(a2) Q90 (mm/d): Hybrid (Mean) − Default Parameter', 'FontSize', title_size);
set(gca,'FontSize',tick_size);
...
caxis([cmin cmax]);  
colorbar
title('(b2) Q90 (mm/d): Hybrid (Mean) − Default Parameter', 'FontSize', title_size);
set(gca, 'XTickLabel', []);
set(gca, 'YTickLabel', []);
set(gca,'FontSize',tick_size);
...
ax(5) = subplot(3,2,5)
metric_daily = out_q90_metric(:,10);
scatter(basin_lon, basin_lat, scatter_size, metric_daily, 'filled', 'MarkerEdgeColor',[105 105 105]/255, 'LineWidth',line_width-0.5); hold on; 
grid on;
set(gca, 'GridLineStyle', ':');
...
colorbar
caxis([0 3]); 
title('(a3) CV of Q90: Hybrid', 'FontSize', title_size);
set(gca,'FontSize',tick_size);
...
caxis([0 3]); 
colorbar
title('(b3) CV of Q90: Hybrid', 'FontSize', title_size);
% set(gca, 'XTickLabel', []);
set(gca, 'YTickLabel', []);
set(gca,'FontSize',tick_size);
xlim([-128 -64]); ylim([23 52]); box on;
