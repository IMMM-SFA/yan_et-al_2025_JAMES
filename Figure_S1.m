...
for i = 1:464
    ...
    if out_q10_metric(i,1) == 1144000
        regional_best_c1 = temp2;
    end
    if out_q10_metric(i,1) == 14216500
        regional_best_c2 = temp2;
    end
    if out_q10_metric(i,1) == 9430500
        regional_best_c3 = temp2;
    end
    if out_q10_metric(i,1) == 9312600
        regional_best_c4 = temp2;
    end    
    if out_q10_metric(i,1) == 8195000
        regional_best_c5 = temp2;
    end    
    if out_q10_metric(i,1) == 5507600
        regional_best_c6 = temp2;
    end
    if out_q10_metric(i,1) == 2363000
...
[f,xi] = ksdensity(regional_best_c1, 'Bandwidth', 0.01); 
fill([xi fliplr(xi)], [f zeros(size(f))], 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'none'); hold on;
% plot(xi,f); hold on; 
[f,xi] = ksdensity(regression_c1, 'Bandwidth', 0.08); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [34 139 34]/255, 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
[f,xi] = ksdensity(hybrid_c1, 'Bandwidth', 0.08); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [0.8500 0.3250 0.0980], 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;

% plot(xi,f); hold on;
xline(out_obs(index, 5), '--g', 'Linewidth', line_width+1); 
xline(out_default(index, 5), '--r', 'Linewidth', line_width+1);  
% xline(out_regional(index, 7), '--g', 'Linewidth', line_width+1); 
xlabel('Q10 (mm/d)', 'FontSize', label_size);  
...
text(0.05, 0.80, 'CRPSS R-S = 0.41', 'FontSize',text_size,  'Units','Normalized')
text(0.05, 0.70, 'CRPSS Hybrid = 0.12', 'FontSize',text_size,  'Units','Normalized')
ylim([0, 5]);
xlim([0,0.6])
...
gauge_id = 14216500;
index = find(out_qmean_metric(:,1)==gauge_id);
[f,xi] = ksdensity(regional_best_c2, 'Bandwidth', 0.01); 
fill([xi fliplr(xi)], [f zeros(size(f))], 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'none'); hold on;
% plot(xi,f); hold on; 
% [f,xi] = ksdensity(regression_c2, 'Bandwidth', 0.08); 
% fill([xi fliplr(xi)], [f zeros(size(f))],  [34 139 34]/255, 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
[f,xi] = ksdensity(hybrid_c2, 'Bandwidth', 0.01); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [0.8500 0.3250 0.0980], 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
xline(out_obs(index, 5), '--g', 'Linewidth', line_width+1); 
xline(out_default(index, 5), '--r', 'Linewidth', line_width+1);  
% xline(out_regional(index, 7), '--g', 'Linewidth', line_width+1); 
...
% text(0.05, 0.15, '(a)', 'FontSize',text_size, 'FontWeight', 'bold', 'Units','Normalized')
text(0.05, 0.9, 'CRPSS R-P = 0', 'FontSize',text_size, 'Units','Normalized')
text(0.05, 0.80, 'CRPSS R-S = NaN (Failed)', 'FontSize',text_size,  'Units','Normalized')
text(0.05, 0.70, 'CRPSS Hybrid = 0', 'FontSize',text_size,  'Units','Normalized')
ylim([0, 40]);
xlim([0, 1.2]);
...
gauge_id = 09430500;
index = find(out_qmean_metric(:,1)==gauge_id);
[f,xi] = ksdensity(regional_best_c3, 'Bandwidth', 0.001); 
fill([xi fliplr(xi)], [f zeros(size(f))], 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'none'); hold on;
% plot(xi,f); hold on; 
% [f,xi] = ksdensity(regression_c2, 'Bandwidth', 0.08); 
% fill([xi fliplr(xi)], [f zeros(size(f))],  [34 139 34]/255, 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
[f,xi] = ksdensity(hybrid_c3, 'Bandwidth', 0.001); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [0.8500 0.3250 0.0980], 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
xline(out_obs(index, 5), '--g', 'Linewidth', line_width+1); 
xline(out_default(index, 5), '--r', 'Linewidth', line_width+1);  
% xline(out_regional(index, 7), '--g', 'Linewidth', line_width+1); 
...
gauge_id = 09312600;
index = find(out_qmean_metric(:,1)==gauge_id);
[f,xi] = ksdensity(regional_best_c4, 'Bandwidth', 0.001); 
fill([xi fliplr(xi)], [f zeros(size(f))], 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'none'); hold on;
% plot(xi,f); hold on; 
[f,xi] = ksdensity(regression_c4, 'Bandwidth', 0.001); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [34 139 34]/255, 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
[f,xi] = ksdensity(hybrid_c4, 'Bandwidth', 0.001); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [0.8500 0.3250 0.0980], 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
...
% text(0.05, 0.15, '(a)', 'FontSize',text_size, 'FontWeight', 'bold', 'Units','Normalized')
text(0.05, 0.9, 'CRPSS R-P = 0', 'FontSize',text_size, 'Units','Normalized')
text(0.05, 0.80, 'CRPSS R-S = 0', 'FontSize',text_size,  'Units','Normalized')
text(0.05, 0.70, 'CRPSS Hybrid = 0', 'FontSize',text_size,  'Units','Normalized')
ylim([0, 40]);
xlim([0, 0.1]);
...
gauge_id = 08195000;
index = find(out_qmean_metric(:,1)==gauge_id);
[f,xi] = ksdensity(regional_best_c5, 'Bandwidth', 0.001); 
fill([xi fliplr(xi)], [f zeros(size(f))], 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'none'); hold on;
% plot(xi,f); hold on; 
% [f,xi] = ksdensity(regression_c2, 'Bandwidth', 0.08); 
% fill([xi fliplr(xi)], [f zeros(size(f))],  [34 139 34]/255, 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
[f,xi] = ksdensity(hybrid_c5, 'Bandwidth', 0.001); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [0.8500 0.3250 0.0980], 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
xline(out_obs(index, 5), '--g', 'Linewidth', line_width+1); 
xline(out_default(index, 5), '--r', 'Linewidth', line_width+1);  
% xline(out_regional(index, 7), '--g', 'Linewidth', line_width+1); 
...
index = find(out_qmean_metric(:,1)==gauge_id);
index = find(out_qmean_metric(:,1)==gauge_id);
[f,xi] = ksdensity(regional_best_c6, 'Bandwidth', 0.01); 
fill([xi fliplr(xi)], [f zeros(size(f))], 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'none'); hold on;
% plot(xi,f); hold on; 
[f,xi] = ksdensity(regression_c6, 'Bandwidth', 0.01); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [34 139 34]/255, 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
[f,xi] = ksdensity(hybrid_c6, 'Bandwidth', 0.01); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [0.8500 0.3250 0.0980], 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
...
legend('R-P', 'R-S', 'Hybrid', 'Default Parameter Sim.', 'Obs.')
set(gca,'FontSize',tick_size);
% text(0.05, 0.15, '(a)', 'FontSize',text_size, 'FontWeight', 'bold', 'Units','Normalized')
text(0.05, 0.9, 'CRPSS R-P = 1.0', 'FontSize',text_size, 'Units','Normalized')
text(0.05, 0.80, 'CRPSS R-S = 1.0', 'FontSize',text_size,  'Units','Normalized')
text(0.05, 0.70, 'CRPSS Hybrid = 1.0', 'FontSize',text_size,  'Units','Normalized')
ylim([0, 40]);
xlim([0, 0.7]);
...
gauge_id = 02363000;
index = find(out_qmean_metric(:,1)==gauge_id);
[f,xi] = ksdensity(regional_best_c7, 'Bandwidth', 0.001); 
fill([xi fliplr(xi)], [f zeros(size(f))], 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'none'); hold on;
% plot(xi,f); hold on; 
% [f,xi] = ksdensity(regression_c2, 'Bandwidth', 0.08); 
% fill([xi fliplr(xi)], [f zeros(size(f))],  [34 139 34]/255, 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
[f,xi] = ksdensity(hybrid_c7, 'Bandwidth', 0.001); 
fill([xi fliplr(xi)], [f zeros(size(f))],  [0.8500 0.3250 0.0980], 'FaceAlpha', 0.3, 'EdgeColor', 'none'); hold on;
xline(out_obs(index, 5), '--g', 'Linewidth', line_width+1); 
xline(out_default(index, 5), '--r', 'Linewidth', line_width+1);  
% xline(out_regional(index, 7), '--g', 'Linewidth', line_width+1); 
